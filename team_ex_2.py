...
def wiki_sequentially(search, dir_path):
	...
	for item in results:
		try:
			...
			print(f"Error Skipping '{item}' (disambiguation)")
			continue
		...
		print(f"writing to wiki_dl/{out_filename}")
...
def concurrent_threads(search, dir_path):
...
def dl_and_save_process(item, dir_path): # moved to module level in this example due to processes not sharing memory
	try:
		page = wikipedia.page(item, auto_suggest=False)
	except wikipedia.exceptions.DisambiguationError as e:
		print(f"Error Skipping '{item}' (disambiguation)")
...
def concurrent_process(search, dir_path):
	...
	with ProcessPoolExecutor() as executor:
		executor.map(dl_and_save_process, results, [dir_path] * len(results))
...
def main():
	...
	if len(search) < 4: search = "generative artificial intelligence"

	wiki_sequentially(search, dir_path)
	concurrent_threads(search, dir_path)
	concurrent_process(search, dir_path)
	return
